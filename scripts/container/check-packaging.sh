...
require_pattern packaging/docker/Dockerfile.gateway 'go mod edit -replace github\.com/nosway/namrbd=/src/NAMRBD' 'gateway Dockerfile NAMRBD build context replacement'
...
require_pattern packaging/docker/Dockerfile.tools 'go mod edit -replace github\.com/nosway/namrbd=/src/NAMRBD' 'tools Dockerfile NAMRBD build context replacement'
...
require_pattern packaging/docker/compose.sbs-quickstart.yml 'NAMROS_SBS_SERVICE_ENDPOINT: sbs-quickstart-service:9443' 'SBS quickstart gateway service endpoint env'
...
require_pattern packaging/docker/compose.sbs-quickstart.yml 'sbs-quickstart-service:9443' 'SBS quickstart gateway service endpoint'
...
require_pattern packaging/docker/compose.yaml '.*--sbs-data-listen=0\.0\.0\.0:9444' 'SBS raw data 9444 listener'
...
require_pattern packaging/docker/compose.yaml 'NAMROS_SBS_SERVICE_ENDPOINT: sbs-service-lb:9443' 'Community gateway SBS service endpoint env'
...
require_pattern packaging/docker/compose.yaml 'condition: service_started' 'SBS service load balancer does not block on active/standby readyz'
require_pattern packaging/docker/haproxy/sbs-service.cfg 'option httpchk GET /readyz' 'SBS service load balancer readiness check'
require_pattern packaging/docker/haproxy/sbs-service.cfg 'check port 9081' 'SBS service load balancer checks HTTP readiness port'
...
require_pattern packaging/docker/bin/namros-container-community-bootstrap 'NAMROS_COMMUNITY_SBS_SERVICE_LB_SETTLE_SECONDS' 'SBS service load balancer convergence guard'
